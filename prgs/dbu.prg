...
* (JK) clause ON INIT added
   define window _DBU at 0,0 ;
                 ...
                 title "Harbour Minigui DataBase Utility" icon "dbuicon";
                 main ;
...
* (JK) & added to make use with ALT-key shortcut
      ...
      popup "&View"
         item " Browse Mode" + Chr(9)+'Ctrl+B' ;
               action DBUbrowse1()  name _DBUitem9   image 'MENUBROW'
         separator
         item " Edit Mode" + Chr(9)+'Ctrl+E' ;
...
define window DBUsplashwindow at 0,0 ;
              width 400 height 400 topmost nocaption   ;
...
if len(_DBUcondition1) == 0
   ...
   if Type("&_DBUcondition1") # "U"
      ...
      go top
      if eof()
         msgInfo("No Records Matching Your Query!","Warning")
         set filter to
         _DBUfiltered := .f.
         _DBUcondition := ""
      else
      ...
      msgInfo("Wrong Query Condition!","Warning")
   endif
endif
return nil
...
if len(alltrim(_DBUvalue)) == 0
   msginfo("You have to enter the value to be replaced.","DBU Replace")
...
do case
   ...
   case _DBUreplace._DBUoptions.value == 3
      if _DBUreplace._DBUnextrecords.value <= 0
         msginfo("You have to enter the number of records to replace!","DBU Replace")
...
local nI, nFields, aStruct, cOldDbf, xField, lOk, cMess, lOpen
local cNewDBF, CurrArea, cTitle
